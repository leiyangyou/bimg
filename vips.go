...
func vipsWatermark(image *C.struct__VipsImage, w Watermark) (*C.struct__VipsImage, error) {
	...
	maskBackground := [3]C.double{C.double(w.Mask.R), C.double(w.Mask.G), C.double(w.Mask.B)}

	textOpts := vipsWatermarkTextOptions{text, font}
	opts := vipsWatermarkOptions{C.int(w.Width), C.int(w.LineSpacing), C.int(w.DPI), C.int(w.Margin), C.int(noReplicate), C.float(w.Opacity), background, C.float(w.MaskOpacity), maskBackground}
